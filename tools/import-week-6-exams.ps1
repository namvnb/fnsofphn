...
if (-not $SourceDir) {
  $externalSourceDir = "E:\tool\01. CODE\02 fnsofphn\project content\de tuan 6"
  $SourceDir = if (Test-Path -LiteralPath $externalSourceDir) {
    $externalSourceDir
  } else {
    Join-Path $repo "project content\De thi giup Cy\de tuan 6"
  }
}

$publicRoot = Join-Path $repo "public\exam-assets\week-6"
$dataPath = Join-Path $repo "features\giup-cy\week-6-exams.json"
$assetPath = Join-Path $repo "features\giup-cy\week-6-assets.json"
...
function Add-QuestionImages($paragraph, $zip, $relMap, $nsm, [string]$mediaDir, [string]$slug, [int]$questionNumber, $questionImages) {
  foreach ($blip in $paragraph.SelectNodes(".//a:blip", $nsm)) {
    ...
    $questionImages[$questionNumber].Add([ordered]@{
      pageNumber = 1
      url = "/exam-assets/week-6/$slug/media/${outName}?v=week-6-20260608"
      width = $size.width
      height = $size.height
    })
  }
...
foreach ($file in $files) {
  ...
  try {
    ...
    for ($paragraphIndex = 0; $paragraphIndex -lt $paragraphs.Count; $paragraphIndex++) {
      ...
      $sectionIIMatch = [regex]::Match($clean, "PH.{0,3}N\s*II", [Text.RegularExpressions.RegexOptions]::IgnoreCase)
      $sectionIIIMatch = [regex]::Match($clean, "PH.{0,3}N\s*III", [Text.RegularExpressions.RegexOptions]::IgnoreCase)
...
node (Join-Path $PSScriptRoot "postprocess-week-6-exams.mjs")

Write-Host "Generated $($exams.Count) week 6 exams from $SourceDir -> $dataPath"
