...
if (-not $SourceDir) {
  $externalSourceDir = "E:\tool\01. CODE\02 fnsofphn\project content\de tuan 3"
  $SourceDir = if (Test-Path -LiteralPath $externalSourceDir) {
    $externalSourceDir
  } else {
    Join-Path $repo "project content\De thi giup Cy\de tuan 3"
  }
}

$publicRoot = Join-Path $repo "public\exam-assets\week-3"
$dataPath = Join-Path $repo "features\giup-cy\week-3-exams.json"
$assetPath = Join-Path $repo "features\giup-cy\week-3-assets.json"
...
function Add-QuestionImages($paragraph, $zip, $relMap, $nsm, [string]$mediaDir, [string]$slug, [int]$questionNumber, $questionImages) {
  foreach ($blip in $paragraph.SelectNodes(".//a:blip", $nsm)) {
    ...
    $questionImages[$questionNumber].Add([ordered]@{
      pageNumber = 1
      url = "/exam-assets/week-3/$slug/media/${outName}?v=week-3-20260525"
      width = $size.width
      height = $size.height
    })
  }
