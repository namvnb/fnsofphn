...
if (-not $SourceDir) {
  $externalSourceDir = "E:\tool\01. CODE\02 fnsofphn\project content\de tuan 4"
  $SourceDir = if (Test-Path -LiteralPath $externalSourceDir) {
    $externalSourceDir
  } else {
    Join-Path $repo "project content\De thi giup Cy\de tuan 4"
  }
}

$publicRoot = Join-Path $repo "public\exam-assets\week-4"
$dataPath = Join-Path $repo "features\giup-cy\week-4-exams.json"
$assetPath = Join-Path $repo "features\giup-cy\week-4-assets.json"
...
function Add-QuestionImages($paragraph, $zip, $relMap, $nsm, [string]$mediaDir, [string]$slug, [int]$questionNumber, $questionImages) {
  foreach ($blip in $paragraph.SelectNodes(".//a:blip", $nsm)) {
    ...
    $questionImages[$questionNumber].Add([ordered]@{
      pageNumber = 1
      url = "/exam-assets/week-4/$slug/media/${outName}?v=week-4-20260601"
      width = $size.width
      height = $size.height
    })
  }
...
node (Join-Path $PSScriptRoot "postprocess-week-4-exams.mjs")

Write-Host "Generated $($exams.Count) week 4 exams from $SourceDir -> $dataPath"
